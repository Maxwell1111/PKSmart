#!/usr/bin/env python3
"""
Train Baseline Rat PK Models (WITHOUT CE50 Features)

This script trains Random Forest models for rat pharmacokinetic parameters
using ONLY Morgan fingerprints and Mordred descriptors (NO CE50 features).
This serves as a baseline comparison for the CE50-enhanced models.
...
print("="*80)
print("BASELINE RAT MODEL TRAINING (NO CE50 FEATURES)")
print("="*80)
print()
...
# Save feature list
with open("features_mfp_mordred_columns_rat_model_baseline.txt", "w") as f:
    for item in features_mfp_mordred_columns:
        f.write(item + "\n")
print(f"  Saved feature list to: features_mfp_mordred_columns_rat_model_baseline.txt")
...
for endpoint in endpoints:
    ...
    baseline = 10.00
    data = rat_mfp_Mordred  # NO CE50 data
    features = features_mfp_mordred_columns  # NO CE50 features
    ...
    with open('scaler_rat_baseline.pkl', 'wb') as file:
    ...
    for i in range(42, 47):
        ...
        for split, (train_index, test_index) in enumerate(outercv.split(X)):
            ...
            if gmfe < baseline:
                baseline = gmfe
                pickle.dump(classifier, open(f"log_{endpoint}_model_baseline.sav", 'wb'))
...
results.to_csv("Prediction_rat_from_mordred_morgan_fs_baseline.csv", index=False)
print("  Saved detailed results to: Prediction_rat_from_mordred_morgan_fs_baseline.csv")
...
for endpoint in endpoints:
    ...
    loaded_rf = pickle.load(open(f"log_{endpoint}_model_baseline.sav", 'rb'))
    ...
    # Save final model
    pickle.dump(classifier, open(f"log_{endpoint}_model_baseline_FINAL.sav", 'wb'))
    print(f"    Saved: log_{endpoint}_model_baseline_FINAL.sav")
    print(f"    Hyperparameters: {params}")

# Save final scaler (from last endpoint)
print(f"\n  Saved final scaler: scaler_rat_baseline.pkl")
...
print("\n" + "="*80)
print("Step 8: Creating Summary File")
print("="*80)

# Create a simple summary CSV for easy comparison
summary = results.groupby("endpoint").agg({
    "fold_2": "mean",
    "fold_3": "mean",
    "fold_5": "mean",
    "gmfe": "mean",
...
summary.to_csv("rat_baseline.csv")
print("  Saved summary statistics to: rat_baseline.csv")
...
print("\n" + "="*80)
print("SUMMARY STATISTICS (BASELINE - NO CE50)")
print("="*80)
...
print("="*80)
print("\nGenerated files:")
print("  1. features_mfp_mordred_columns_rat_model_baseline.txt - Feature list (539 features)")
print("  2. scaler_rat_baseline.pkl - Feature scaler")
print("  3. log_rat_VDss_L_kg_model_baseline_FINAL.sav - VDss model")
print("  4. log_rat_CL_mL_min_kg_model_baseline_FINAL.sav - CL model")
print("  5. log_rat_fup_model_baseline_FINAL.sav - fup model")
print("  6. Prediction_rat_from_mordred_morgan_fs_baseline.csv - Detailed CV results")
print("  7. rat_baseline.csv - Summary statistics")
print("\nTotal features: 539 (386 Mordred + 153 Morgan, NO CE50)")
print("="*80)
